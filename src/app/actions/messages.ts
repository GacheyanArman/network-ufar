...
export async function sendMessage(formData: FormData) {
  ...
  const rate = checkRateLimit(senderId, "sendMessage");
  if (!rate.allowed) throw new Error(getRateLimitError(rate.resetTime));

  const receiverId = normalize(formData.get("receiverId"));
  const groupChatId = normalize(formData.get("groupChatId"));
  const content = normalize(formData.get("content")).slice(0, MAX_MESSAGE_LEN);
  ...
  if (attachment && typeof attachment === "object" && "size" in attachment && attachment.size > 0) {
    ...
    attachmentUrl = await saveUploadFile(file, {
      subdir: "messages",
      prefix: "msg",
      maxSize: isImage ? 5 * 1024 * 1024 : 10 * 1024 * 1024,
      allowedMimePrefix: isImage ? "image/" : undefined,
    });
...
export async function editMessage(formData: FormData) {
  ...
  if (!content) throw new Error("Cannot save empty message");
...
export async function deleteMessage(formData: FormData) {
  const userId = await requireUserId();
  const messageId = normalize(formData.get("messageId"));
  const forEveryone = formData.get("forEveryone") === "true";
  if (!messageId) throw new Error("Invalid message");
...
export async function markThreadRead(formData: FormData) {
  const userId = await requireUserId();
  const otherUserId = normalize(formData.get("userId"));
