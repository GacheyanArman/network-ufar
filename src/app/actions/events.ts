...
import { isStaff, getUserRole } from "@/lib/roles";
import { eventSchema, eventCommentSchema } from "@/lib/validations";
...
export async function createEvent(formData: FormData) {
  ...
  const rl = checkRateLimit(userId, "createEvent");
  ...
  if (coverFile && coverFile.size > 0) {
    coverUrl = await saveUploadFile(coverFile, {
      subdir: "events",
      prefix: "cover",
  ...
  try {
    ...
    await db
      ...
      .set({
        ...
        endTime: nextEndTime,
        ...(coverUrl !== undefined
          ? { imageUrl: coverUrl, coverImageUrl: coverUrl }
          : {}),
...
export async function createEventComment(formData: FormData) {
  ...
  const rl = checkRateLimit(userId, "createComment");
  if (!rl.allowed) return { error: getRateLimitError(rl.resetTime!) };
