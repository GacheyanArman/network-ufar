...
const EXAM_YEARS = Array.from({ length: 6 }, (_, i) => String(new Date().getFullYear() - i));

const FILE_FORMATS: { value: string; label: string; icon: string; extensions: string[] }[] = [
  { value: "pdf", label: "PDF", icon: "file-pdf", extensions: [".pdf"] },
  { value: "doc", label: "DOCX / DOC", icon: "file-doc", extensions: [".doc", ".docx", ".odt", ".rtf"] },
  { value: "image", label: "Image", icon: "file-image", extensions: [".jpg", ".jpeg", ".png", ".gif", ".webp", ".svg"] },
  { value: "video", label: "Video", icon: "file-video", extensions: [".mp4", ".mov", ".avi", ".mkv", ".webm"] },
  { value: "archive", label: "Archive", icon: "file-archive", extensions: [".zip", ".rar", ".7z", ".tar", ".gz"] },
];
...
const SORTS: { value: MaterialSort; label: string; icon: string }[] = [
  ...
  { value: "most_discussed", label: "Most discussed", icon: "message-circle" },
  { value: "most_downloaded", label: "Most downloaded", icon: "download" },
  { value: "most_viewed", label: "Most viewed", icon: "eye" },
];

function getFileTypeIcon(fileUrl: string | null): string {
  if (!fileUrl) return "file";
  const ext = fileUrl.toLowerCase().split(".").pop() || "";
  for (const fmt of FILE_FORMATS) {
    if (fmt.extensions.some((e) => e.slice(1) === ext)) return fmt.icon;
  }
  return "file-text";
}

function getFileFormatLabel(fileUrl: string | null): string {
  if (!fileUrl) return "File";
  const ext = fileUrl.toLowerCase().split(".").pop() || "";
...
function matchesFileFormat(fileUrl: string | null, format: string): boolean {
  if (!format || !fileUrl) return !format;
  const ext = "." + (fileUrl.toLowerCase().split(".").pop() || "");
  const fmt = FILE_FORMATS.find((f) => f.value === format);
  return fmt ? fmt.extensions.includes(ext) : false;
}
...
export default function MaterialsPageClient({
  ...
  const [filterFileFormat, setFilterFileFormat] = useState("");
  ...
  const filteredMaterials = useMemo(() => {
    let list = materials.filter((m) => {
      ...
      if (filterProfessor && (!m.professorCourse || !m.professorCourse.toLowerCase().includes(filterProfessor.toLowerCase()))) return false;
      if (filterFileFormat && !matchesFileFormat(m.fileUrl, filterFileFormat)) return false;
      return true;
    });
    ...
    list = [...list].sort((a, b) => {
      switch (sort) {
        ...
        case "most_saved": {
          const as = a.isSaved ? 1 : 0;
          const bs = b.isSaved ? 1 : 0;
          return bs - as || new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime();
        }
        case "most_discussed":
  ...
  const hasActiveFilters =
    !!(searchQuery || filterFaculty || filterCourse || filterYear || filterSubject || filterType || filterProfessor || filterSemester || filterFileFormat || filterExamYear) ||
    sort !== "newest";
          ...
          >
            <div className="card materials-filters-card">
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                ...
                <FilterSelect
                  label="File format"
                  value={filterFileFormat}
                  onChange={setFilterFileFormat}
                  options={[
                    { value: "", label: "All formats" },
                    ...FILE_FORMATS.map((f) => ({ value: f.value, label: f.label })),
                  ]}
                />
                <FilterSelect
                  label="Exam year"
                  value={filterExamYear}
                  onChange={setFilterExamYear}
                  options={[{ value: "", label: "Any year" }, ...EXAM_YEARS.map((y) => ({ value: y, label: y }))]}
                />
          ...
          <section style={{ minWidth: 0 }}>
            ...
            {filteredMaterials.length === 0 ? (
              <EmptyState
                ...
                suggestions={[
                  "Try clearing some filters for broader results",
                  "Upload your own notes to help the community",
                  "Request the material you need — other students might have it",
                ]}
              />
            ) : (
...
function MaterialCard({
  ...
  const fileIcon = getFileTypeIcon(m.fileUrl);
  const formatLabel = getFileFormatLabel(m.fileUrl);
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: 14,
        border: m.isVerified ? "1px solid rgba(22,163,74,0.25)" : "1px solid var(--border-color)",
    ...
    >
      ...
      <div style={{ padding: "16px 18px 10px", background: "linear-gradient(135deg, var(--bg-hover) 0%, var(--bg-main) 100%)", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 40, height: 40, borderRadius: 10, background: "rgba(var(--primary-rgb), 0.1)", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--primary-color)", flexShrink: 0 }}>
            <UiIcon name={fileIcon} size={20} />
          </div>
          <div>
            <span style={{ fontSize: "0.65rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.6px", color: "var(--text-secondary)" }}>{formatLabel}</span>
            <div style={{ fontSize: "0.7rem", fontWeight: 600, color: "var(--primary-color)", textTransform: "uppercase", letterSpacing: "0.4px" }}>{formatType(m.type)}</div>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          {m.isVerified && (
            <span title="Verified &amp; approved" style={{ display: "inline-flex", alignItems: "center", gap: 4, background: "linear-gradient(135deg, #16a34a, #15803d)", color: "var(--bg-card)", padding: "3px 8px", borderRadius: 20, fontSize: "0.63rem", fontWeight: 700, letterSpacing: "0.3px", textTransform: "uppercase" }}>
              <UiIcon name="shield-check" size={11} /> Verified
            </span>
          )}
          <button onClick={onToggleSave} aria-label={m.isSaved ? "Unsave" : "Save"} style={{ background: "none", border: "none", cursor: "pointer", color: m.isSaved ? "var(--primary-color)" : "var(--text-secondary)", transition: "color 0.2s", padding: 2, lineHeight: 0 }}>
      ...
      {/* Body */}
      <div style={{ padding: "6px 18px 0", display: "flex", flexDirection: "column", flex: 1, gap: 8 }}>
        <h3 style={{ fontSize: "1.05rem", margin: 0, lineHeight: 1.35, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
          {m.title}
        </h3>
        {m.description && (
          <p style={{ fontSize: "0.85rem", color: "var(--text-secondary)", margin: 0, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden", minHeight: "2.4em" }}>
            {m.description}
          </p>
        )}
        <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
          ...
          {m.professorCourse && <MetaChip icon="user">{m.professorCourse}</MetaChip>}
        </div>
        <div onClick={(e) => e.stopPropagation()} style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <StarRating value={m.myRating || Math.round(m.averageRating)} onChange={onRate} />
          <span style={{ fontSize: "0.8rem", color: "var(--text-secondary)" }}>
            {m.ratingCount > 0 ? `${m.averageRating.toFixed(1)} (${m.ratingCount})` : "Be the first to rate"}
          </span>
        </div>
      </div>

      {/* Footer */}
      <div style={{ marginTop: 10, padding: "10px 18px", borderTop: "1px solid var(--border-color)", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
        <div style={{ display: "inline-flex", alignItems: "center", gap: 10, color: "var(--text-secondary)", fontSize: "0.78rem" }}>
          <span style={{ display: "inline-flex", alignItems: "center", gap: 3 }}><UiIcon name="eye" size={13} />{formatNumber(m.viewsCount)}</span>
          <span style={{ display: "inline-flex", alignItems: "center", gap: 3 }}><UiIcon name="download" size={13} />{formatNumber(m.downloadsCount)}</span>
          {m.ownerName && <span style={{ display: "inline-flex", alignItems: "center", gap: 3 }}><UiIcon name="user" size={13} />{m.ownerName.split(" ")[0]}</span>}
        </div>
        <button onClick={(e) => { e.stopPropagation(); onDownload(); }} className="btn btn-primary" style={{ padding: "6px 12px", fontSize: "0.82rem", display: "inline-flex", alignItems: "center", gap: 5, borderRadius: 6, border: "none", cursor: "pointer" }} disabled={!m.fileUrl}>
          <UiIcon name="download" size={13} /> Get
        </button>
      </div>
    </div>
  );
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: 14,
        gap: 14,
        background: "linear-gradient(180deg, var(--bg-main) 0%, var(--bg-hover) 100%)",
      }}
    >
      <div style={{ width: 72, height: 72, borderRadius: "50%", background: "rgba(var(--primary-rgb), 0.08)", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--primary-color)" }}>
        <UiIcon name={icon} size={36} />
      </div>
      <h3 style={{ margin: 0, color: "var(--text-primary)", fontSize: "1.15rem" }}>{title}</h3>
      <p style={{ color: "var(--text-secondary)", margin: 0, maxWidth: 440, lineHeight: 1.5 }}>{hint}</p>
      {suggestions && suggestions.length > 0 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: 4, width: "100%", maxWidth: 400 }}>
          {suggestions.map((s, i) => (
            <div
              key={i}
              style={{
            ...
            >
              <UiIcon name="lightbulb" size={16} color="var(--primary-color)" />
              <span>{s}</span>
            </div>
          ))}
        </div>
      ...
      {actionLabel && onAction && (
        <button
          className="btn btn-primary"
          onClick={onAction}
          style={{ marginTop: 8, padding: "10px 20px", borderRadius: 8, display: "inline-flex", alignItems: "center", gap: 6 }}
        >
...
function UrgencyBadge({ urgency }: { urgency: string }) {
  const palette: Record<string, { bg: string; color: string }> = {
    high: { bg: "rgba(220, 38, 38, 0.12)", color: "var(--danger)" },
    medium: { bg: "rgba(245, 158, 11, 0.12)", color: "#b45309" },
    low: { bg: "rgba(22, 163, 74, 0.12)", color: "var(--success)" },
  };
      ...
      >
        ...
        >
          <div>
            ...
            {material.isVerified && (
              <span
                title="Verified &amp; approved by moderators"
                style={{
                  display: "inline-flex",
                  alignItems: "center",
                  gap: 5,
