...
import { eq, and, desc, inArray, asc, gte, isNull } from "drizzle-orm";
import {
...
import type { UnreadConversation } from "./UnreadMessagesCard";
...
export default async function TodayDashboard({
  ...
  // ── Unread messages ────────────────────────────────────────────────────
  const [unreadMessages, unreadRows] = await Promise.all([
  ...
  // One row per sender (latest unread message as preview)
  const unreadConversations: UnreadConversation[] = [];
  for (const row of unreadRows) {
    if (unreadConversations.some((c) => c.senderId === row.senderId)) continue;
    unreadConversations.push({
      senderId: row.senderId,
      senderName: row.senderName,
      preview:
        row.content.slice(0, 64) + (row.content.length > 64 ? "\u2026" : ""),
    });
    if (unreadConversations.length >= 3) break;
  }
