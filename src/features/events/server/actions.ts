...
<<<<<<< HEAD
    case "my_courses":
      ...
      ).map((c: any) => c.courseId).filter(Boolean);
      if (enrolledCourseIds.length > 0) {
        conditions.push(inArray(events.courseId, enrolledCourseIds));
      } else {
        return [];
      }
      break;
    case "university":
      conditions.push(ongoingOrFuture);
      conditions.push(isNull(events.communityId));
      conditions.push(isNull(events.courseId));
      break;
    case "clubs_groups":
      conditions.push(ongoingOrFuture);
